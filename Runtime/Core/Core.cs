...
namespace CrowdMorph
{
   ...
   unsafe struct SampleClipInstanceJob : IJobParallelFor
   {
      ...
      public NativeArray<int> BoneToBindingIndex;

      [ReadOnly, DeallocateOnJobCompletion]
      public NativeArray<BoneToBindingIndirectMap> BoneToBindingIndirectMap;

      public int InverseMatrixBufferOffset;
      ...
      {
         int bindingIndex = BoneToBindingIndex[boneIndex];

         if (bindingIndex == -1)
            return BoneToBindingIndirectMap[boneIndex].StaticOffset;

         int i = frameIndex * Clip.Value.Bindings.Length + bindingIndex;
         return new AffineTransform(Clip.Value.LocalTranslations[i], Clip.Value.LocalRotations[i], Clip.Value.LocalScales[i]);
      }
   }
   ...
   public static partial class Core
   {
      ...
      public static bool EvaluateTransitionExitTime(ref Transition transition, float averageMotionLength, float previousTime, float currentTime)
      {
         if (transition.HasExitTime)
         {
            ...
            float normalizedPreviousTimeMod = math.fmod(normalizedPreviousTime, averageMotionLength);
            float normalizedCurrentTimeMod = math.fmod(normalizedCurrentTime, averageMotionLength);
            return Overlaps(normalizedPreviousTimeMod, normalizedCurrentTimeMod, transition.ExitTime);
      ...
      )
      {
         ...
         var handle = new SampleClipInstanceJob
         {
            ...
            Skeleton = skeleton,
            OutSamples = samples,
            BoneToBindingIndex = boneToBindingIndex,
            BoneToBindingIndirectMap = boneToBindingIndirectMap,
      ...
      )
      {
         ...
         for (int i = 0; i < skeleton.Value.BoneCount; i++)
         {
            outBoneToBindingIndex[i] = FindBindingIndex(ref clip.Value.Bindings, skeleton.Value.BoneIDs[i]);
