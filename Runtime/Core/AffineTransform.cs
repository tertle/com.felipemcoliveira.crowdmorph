...
namespace CrowdMorph
{
   ...
   public struct AffineTransform
   {
      ...
      public string DebugTRS
      {
         get
         {
            var scale = new float3(math.length(RotationScale.c0), math.length(RotationScale.c1), math.length(RotationScale.c2));
            var rotation = (UnityEngine.Quaternion)new quaternion(new float3x3(new float3(RotationScale.c0 / scale.x), new float3(RotationScale.c1 / scale.y), new float3(RotationScale.c1 / scale.y)));

            return string.Format("T{0:0.##} R{1:0.##}, S{2:0.##}", (UnityEngine.Vector3)Translation, rotation.eulerAngles, (UnityEngine.Vector3)scale);
         }
      }
