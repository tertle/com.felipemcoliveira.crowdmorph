...
namespace CrowdMorph
{
   internal unsafe struct AnimationCommandListData : IDisposable
   {
      ...
      {
         var dataPtr = (AnimationCommandListData*)UnsafeUtility.Malloc(sizeof(AnimationCommandListData), sizeof(void*), allocator);

         dataPtr->PassCount = passCount;
         dataPtr->InternalPassCapacity = -1;
         dataPtr->m_Allocator = allocator;
         dataPtr->m_PassLengths = null;
         dataPtr->m_PassBuffers = null;

         dataPtr->SetInternalPassCapacity(initialPassCapacity);
